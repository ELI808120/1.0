...
import { ArrowRight, PlusCircle, Mail, Loader } from 'lucide-react';
...
const CoursePage: React.FC = () => {
  ...
  // FIX: Destructure `profile` from useAuth to check for course access.
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Loader className="animate-spin text-[var(--theme-color-500)]" size={48} />
      </div>
    );
  }
  ...
  // FIX: Check for access using the `hasPaid` flag from the user's Firestore profile.
  const hasAccess = profile?.hasPaid || false;

  if (!hasAccess && !isAdmin) {
      return <PurchasePrompt />;
  }

  // Admin always has access for editing purposes.
  if (!hasAccess && isAdmin) {
      console.log("Admin previewing course content.");
  }
